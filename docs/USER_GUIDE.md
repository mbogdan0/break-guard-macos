...
The overlay's postpone buttons are hold-to-confirm: keep one pressed until its fill completes — one second for the shorter postponement, three for the longer. Each button states its own hold length at its right edge, so there is nothing to discover by trial. With **Harder to skip breaks** enabled the holds are longer from the start — three seconds and nine — and each cycle grants only one skip action (an extension or a postponement); once it is spent, further postponements are blocked outright rather than made costlier. In normal mode a second postponement within the same cycle costs the same three and nine seconds.

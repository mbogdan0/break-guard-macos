...
The overlay's postpone buttons are hold-to-confirm: keep one pressed until its fill completes — one second for the shorter postponement, three for the longer. Each button states its own hold length underneath its label, so there is nothing to discover by trial. With **Harder to skip breaks** enabled, each cycle grants one skip action (an extension or a postponement) at normal cost; after it is spent, every further postponement demands a doubled hold.
